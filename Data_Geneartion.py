...
for p_i in range(2,3):
    ...
    sys = Simulation_VeryLarge(barcodes=barcodes, cells=cells, generations=1)

    system_true  = barcodes_id_list(system_true.copy())
    gens1 = 5
    system2 = sys.run(system_true, gens1)
    df['propagation_' + str(gens1) + 'geneartions_true'] =system2

    gens2 = 5
    system4 = sys.run(system2, gens2 )
    df['propagation_' + str(gens1+gens2) + 'geneartions_true'] = system4

    gens3 = 5
    system6 = sys.run(system4, gens3)
    df['propagation_' + str(gens1+gens2+gens3) + 'geneartions_true'] = system6
    ...
    for p_d in range(3,4):
        ...
        system_true_drop = sys.dropouts(system_true, p=p_drop)
        df['drop_no_propagation'] = system_true_drop

        system2_drop = sys.dropouts(system2, p=p_drop)
        df['drop_' + str(gens1) + 'generation'] = system2_drop

        system4_drop = sys.dropouts(system4, p=p_drop)
        df['drop_' + str(gens1 + gens2) + 'generation'] = system4_drop

        system6_drop = sys.dropouts(system6, p=p_drop)
        df['drop_' + str(gens1 + gens2 + gens3) + 'generation'] = system6_drop
